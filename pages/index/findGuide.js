...
Page({
  ...
  pub: function () {
    var startTime = this.data.startDate, endTime = this.data.endDate, price = this.data.price, experience = this.data.experience, language = this.data.language,
      userId = wx.getStorageSync('userId'), area = this.data.address.resideprovince + '-' + this.data.address.residecity 
      console.log(area)
    wx.request({
      url: config.host +'/Publish/seekGuide',
      data:{
        startTime: startTime,
        endTime: endTime,
        price: price,
